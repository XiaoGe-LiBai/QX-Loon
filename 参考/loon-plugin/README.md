...
每个脚本版本**首次命中时**会弹一条一次性诊断通知（每个版本只弹一次，不刷屏）：

```
标题:   京东脚本已加载 v2026-09-16.3
...
| 通知表现 | 结论 |
|---|---|
| **压根没弹** | 脚本没跑起来 → 问题在 MITM / 规则匹配 / 远程脚本加载，**不在脚本本身** |
| 弹了但 `pt_key ✗`，且**字段(n)** 的 n 很小 | 代理只把部分同名 cookie 头交出来了 → 需要换拦截目标或改抓取策略 |
| 弹了且 `pt_key ✓` | 脚本与提取都正常 → 若服务器仍没收到，问题在出网（上报那一步） |

> 通知里只列**字段名**，不含任何凭据值。
> 每个版本弹一次：改了脚本、版本号变了才会再弹一次。

### 第二步：看日志

日志每条都带版本号（`[京东凭据 v2026-09-16.3]`）。**先确认跑的是哪一版** —— Loon 会缓存远程脚本，改完脚本若版本号没变，说明加载的还是旧缓存；把插件开关切一次、或删除重新添加可强制重拉。
