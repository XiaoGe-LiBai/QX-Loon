...
日志每条都带版本号（`[京东凭据 v2026-09-16.2]`）。**先确认跑的是哪一版** —— Loon 会缓存远程脚本，改完脚本若版本号没变，说明加载的还是旧缓存；把插件开关切一次、或删除重新添加可强制重拉。

脚本默认**限流打日志**（10 秒最多 1 条），所以在 Loon 脚本日志里：

- **完全没有任何输出** → 脚本没跑起来，问题在 MITM / 规则匹配 / 远程脚本拉取，不在脚本本身。
- **有 `命中 ...` 但显示 `pt_key ✗`** → 命中了但没抓到凭据。日志同时给出 cookie 头个数，可判断是"该请求本来就不带 CK"还是"只拿到了一部分头"。
- **有 `==== [京东凭据捕获 v...] ====`** → 已抓到凭据并外发，紧跟着打印上报结果。
